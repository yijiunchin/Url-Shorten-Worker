        ...
        .then(function (e) {
          if (
            ((res = e),
              ...
              (document.getElementById("addBtn").innerHTML = "縮！"),
              "200" == res.status)
          ) {
            let e = res.key,
...
function addUrlToList(e, t) {
  ...
  // 在 🔗 前面插入換行
  l.innerHTML = window.location.origin + "/" + e + "<br>🔗 " + t;
  o.appendChild(l);
  ...
  c.setAttribute("data-toggle", "popover");
  c.setAttribute("data-placement", "bottom");
  c.setAttribute("data-content", "已複製");
  c.innerText = "📋";
  d.appendChild(c);
  $(c).popover();
  ...
  (document.getElementById("delBtn-" + e).disabled = !0),
    ...
    fetch(window.location.pathname, {
      ...
      .then(function (t) {
        res = t;
        if (res.status == "200") {
          // 成功刪除：移除 localStorage 並重載列表，不顯示 Modal
          localStorage.removeItem(e);
          loadUrlList();
        } else {
          // 刪除失敗：恢復按鈕狀態、顯示錯誤並彈出 Modal
...
function loadKV() {
  (document.getElementById("loadKV2localStgBtn").disabled = !0),
    ...
    fetch(window.location.pathname, {
      ...
      .catch(function (e) {
        alert("未知錯誤，請稍後再試！"), console.log(e);
      })
      .finally(function () {
